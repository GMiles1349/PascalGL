...
//      property Vertex[Index: GLUint]: TPGLVertex read GetVertex;
  ...
  type TPGLAtlas = Class(TPersistent)
    private
  ...
  type TPGLFont = Class(TObject)
    ...
    public
      ...
      constructor Create(AFontName: String; ACreateSDFFont: Boolean = False);
...
SizeRequest: TFTSizeRequest;
  begin
    ...
    Self.PointSize := APointSize;
    Face := TFTFace.Create(AnsiString(AFontName),0);
    Face.SetPixelSize(0, RoundF( (APointSize * (Context.Height / Context.NativeScreenHeight)) * (Context.DPIY / 72)  ));
...
I: GLInt;
  begin
    ...
    if ACreateSDFFont = true then begin
      ...
      Self.fAtlas[0] := TPGLAtlas.Create();
      Self.fAtlas[0].MakeSDF(AFontName,12);

      Self.fAtlas[1] := TPGLAtlas.Create();
      Self.fAtlas[1].MakeSDF(AFontName,16);

      Self.fAtlas[2] := TPGLAtlas.Create();
      Self.fAtlas[2].MakeSDF(AFontName,24);

      Self.fAtlas[3] := TPGLAtlas.Create();
      Self.fAtlas[3].MakeSDF(AFontName,32);

      Self.fAtlas[4] := TPGLAtlas.Create();
      Self.fAtlas[4].MakeSDF(AFontName,48);

      Self.fAtlas[5] := TPGLAtlas.Create();
      ...
      Self.fAtlas[6] := TPGLAtlas.Create();
      Self.fAtlas[6].MakeNormal(AFontName,36);

      Self.fAtlas[7] := TPGLAtlas.Create();
      Self.fAtlas[7].MakeNormal(AFontName,48);

      Self.fAtlas[8] := TPGLAtlas.Create();
      Self.fAtlas[8].MakeNormal(AFontName,60);

      Self.fAtlas[9] := TPGLAtlas.Create();
...
I: GLInt;
  begin
      ...
      Self.fAtlas[0] := TPGLAtlas.Create();
      Self.fAtlas[0].MakeNormal(Self.fFontPath,8);

      Self.fAtlas[1] := TPGLAtlas.Create();
      Self.fAtlas[1].MakeNormal(Self.fFontPath,12);

      Self.fAtlas[2] := TPGLAtlas.Create();
      Self.fAtlas[2].MakeNormal(Self.fFontPath,16);

      Self.fAtlas[3] := TPGLAtlas.Create();
      Self.fAtlas[3].MakeNormal(Self.fFontPath,20);

      Self.fAtlas[4] := TPGLAtlas.Create();
      Self.fAtlas[4].MakeNormal(Self.fFontPath,24);

      Self.fAtlas[5] := TPGLAtlas.Create();
      Self.fAtlas[5].MakeNormal(Self.fFontPath,30);

      Self.fAtlas[6] := TPGLAtlas.Create();
      Self.fAtlas[6].MakeNormal(Self.fFontPath,36);

      Self.fAtlas[7] := TPGLAtlas.Create();
      Self.fAtlas[7].MakeNormal(Self.fFontPath,48);

      Self.fAtlas[8] := TPGLAtlas.Create();
      Self.fAtlas[8].MakeNormal(Self.fFontPath,60);

      Self.fAtlas[9] := TPGLAtlas.Create();
      Self.fAtlas[9].MakeNormal(Self.fFontPath,72);
